...
public class RedisScheduler implements Scheduler {
     ...
     */
    private final static String PREFIX_SET = "Spider.consumer.set";

    /**
     * 根据种子的优先级先简单创建不同的几个队列
     */
    private final static String PREFIX_QUEUE_HIGH = "Spider.queue.consumer.high";
    private final static String PREFIX_QUEUE_LOW = "Spider.queue.consumer.low";
    private final static String PREFIX_QUEUE_DEFAULT = "Spider.queue.consumer.default";
    private final static String PREFIX_QUEUE_CONSUMER = "Spider.queue.consumer.consumer";

    /**
     * @param urlSeed 种子
     * @desciption: 配置 jedisPool
    ...
    public void push(UrlSeed urlSeed) {
        try (Jedis jedis = JedisUtils.getSingleInstance().getJedis()) {
            ...
            if (!jedis.sismember(PREFIX_SET, urlSeed.getUrlHash())) {
                ...
                } else if (urlSeedPriority > 0) {
                    jedis.lpush(PREFIX_QUEUE_LOW, urlSeedToJson);
                } else {
                    jedis.lpush(PREFIX_QUEUE_CONSUMER, urlSeedToJson);
                }
            }
        } catch (Exception e) {
    ...
    public UrlSeed pop() {
        ...
        try {
            if ((urlSeedToJson = jedis.lpop(PREFIX_QUEUE_CONSUMER)) != null) {
                urlSeed = gson.fromJson(urlSeedToJson, UrlSeed.class);
            } else if ((urlSeedToJson = jedis.lpop(PREFIX_QUEUE_HIGH)) != null) {
                urlSeed = gson.fromJson(urlSeedToJson, UrlSeed.class);
