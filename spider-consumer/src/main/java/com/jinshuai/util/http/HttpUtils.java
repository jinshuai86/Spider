...
//            HostnameVerifier hostnameVerifier = SSLConnectionSocketFactory.getDefaultHostnameVerifier();
    ...
    private void sendRequest(String urlStr) {
        ...
        try {
            ...
            switch (statusCode / 100) {
                case 2:
                    executeStrategy(SuccessStrategy.getInstance(), urlStr, response);
                    break;
                case 3:
                    executeStrategy(RedirectStrategy.getInstance(), urlStr, response);
                    break;
                case 4:
                    executeStrategy(ClientErrorStrategy.getInstance(), urlStr, response);
                    break;
                case 5:
                    executeStrategy(ServerErrorStrategy.getInstance(), urlStr, response);
                    break;
            }
        } catch (IOException e) {
    ...
    public static class RedirectStrategy implements StatusHandler {
        ...
        public void process(String url, HttpResponse response) {
            ...
            log.error("301: 资源[{}]已被重定向[{}]", url, location.getValue());
        }
