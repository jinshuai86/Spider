...
public class PriorityQueueScheduler implements Scheduler {
     ...
     * 存储种子的优先队列，采用大根堆实现
     */
    private final PriorityQueue<UrlSeed> urlQueue;
    ...
    public PriorityQueueScheduler(long targetNum) {
        bloomFilter = BloomFilter.create(Funnels.stringFunnel(Charset.forName("UTF-8")), targetNum, 0.01);
...
//            log.warn("url:[{}]已存在", urlSeed.getUrl());
    ...
    public static void main(String[] args) {
        ...
        PriorityQueueScheduler priorityQueueScheduler = new PriorityQueueScheduler(800);
