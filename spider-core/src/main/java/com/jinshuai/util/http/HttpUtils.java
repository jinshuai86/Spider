        ...
        try {
            ...
            producer.send(msg, new SendCallback() {
                ...
                public void onException(Throwable e) {
                    log.error("msg send fail: [{}]", url, e);
                }
            });
        } catch (UnsupportedEncodingException e) {
            log.error("MQ wrong encoding:[{}]", CHARSET, e);
        } catch (InterruptedException | RemotingException | MQClientException e) {
            log.error("Client Or Server Wrong", e);
        }
    }
...
//            scheduler.push(new UrlSeed(location.getValue(), 5));
            ...
            sendMessage(location.getValue(), "Redirect-Topic");
        }
    ...
    public static class ClientErrorStrategy implements StatusHandler {
        ...
        public void process(String url, HttpResponse response) {
            ...
            if (status == 401 || status == 403) {
                ...
                // send to mq
                sendMessage(url, "Forbidden-Topic");
            } else if (status == 404) {
    ...
    public static class ServerErrorStrategy implements StatusHandler {
        ...
        public void process(String url, HttpResponse response) {
            ...
            sendMessage(url, "ServerWrong-Topic");
        }
